...
* anti_consensus：选择最终未见域 agreement 更低的分支；
* pro_consensus：选择最终未见域 agreement 更高的分支；
* random_label：在拟合有效的 sibling 中随机选择标签。

分支选择只读取训练拟合和函数分布指标，不读取任何符号复杂度。符号审计只在
全部路径完成后，以全新初始化重新训练最终数据集时执行。

脚本可整段复制到 AutoDL notebook 单元运行。默认配置是几分钟级 8-bit pilot；
设置环境变量 NSP_SMOKE_TEST=1 可运行本地 CPU 冒烟测试。
"""
...
class Config:
    ...
    RESULT_DIR = Path("/root/results_adversarial_disagreement_completion_pilot")
    RESUME = True
