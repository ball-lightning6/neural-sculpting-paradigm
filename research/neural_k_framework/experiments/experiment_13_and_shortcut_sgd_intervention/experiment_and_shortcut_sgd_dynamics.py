"""
AND shortcut 的多 seed SGD 函数分布实验。

比较原始 n=10 逻辑缺口及其最小改动对照。平衡对照只替换一个样本，
让 AND 的四种 (x1, x2) 模式按 3/2/2/3 覆盖。网络、初始化 seed 和
训练协议严格配对。

目标：判断静态 low-loss prior 中 AND -> F550/F500 的换序，是否也会出现
在真实 SGD 训练诱导的 hard-function 分布中。
"""
...
class Config:
    WIDTH = 64
    HIDDEN_LAYERS = 3
    # 配对比较原始缺口与只替换一个样本后的平衡条件。
    ACTIVE_CONDITIONS = (
        "original_gap_n10",
        "balanced_minimal_n10",
    )
    SEED_COUNT = 2_048
    INITIALIZATION_SEED = 94_000_001

    LEARNING_RATE = 1e-3
    WEIGHT_DECAY = 0.0
    # 50k足以复现两组平台；原始档案另延长至127,458步核验长期稳定。
    MAX_STEPS = 50_000
    EARLY_EVAL_STEPS = (
        0, 1, 2, 5, 10, 20, 50, 100, 200, 500,
    )
    EVAL_INTERVAL_STEPS = 500
    SAVE_INTERVAL_STEPS = 25_000
    SAVE_INTERVAL_SECONDS = 60.0
    RESUME = True

    DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
    ALLOW_TF32 = False
    RESULT_DIR = Path("/root/results_and_shortcut_sgd_intervention_publication")
...
class BatchedIndependentMLP(nn.Module):
    def __init__(self, seed_count: int, condition_count: int) -> None:
        ...
        self.norm_weights = nn.ParameterList([
            nn.Parameter(torch.ones(model_count, Config.WIDTH))
            for _ in range(Config.HIDDEN_LAYERS)
        ])
        self.norm_biases = nn.ParameterList([
            nn.Parameter(torch.zeros(model_count, Config.WIDTH))
            for _ in range(Config.HIDDEN_LAYERS)
        ])
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        for index, (weight, bias) in enumerate(zip(self.weights, self.biases)):
            ...
            if index < len(self.weights) - 1:
                ...
                hidden = (
                    hidden * self.norm_weights[index][:, None, :]
                    + self.norm_biases[index][:, None, :]
                )
...
def print_eval_rows(rows: Sequence[dict[str, Any]]) -> None:
    for row in rows:
        ...
        print(
            ...
            f"F100={row['p_F100']:.2%} F500={row['p_F500']:.2%} "
            f"F510={row['p_F510']:.2%} F550={row['p_F550']:.2%} "
            f"F110={row['p_F110']:.2%} "
...
def main() -> None:
    ...
    print("=== AND shortcut SGD dynamics ===", flush=True)
    max_steps_label = (
    ...
    try:
        ...
        write_json(output_dir / "summary.json", {
            ...
            "seed_count": Config.SEED_COUNT,
            "tracked_functions": [
                "AND_F000", "F100", "F500", "F510",
                "F550", "F700", "F110",
            ],
            "question": (
                "post-interpolation shortcut 是否持续向少例外 AND 漂移。"
            ),
        })
        archive = None
        if Config.PACKAGE_RESULTS:
            archive = shutil.make_archive(
                str(output_dir) + "_package", "zip", output_dir
            )
