"""Parity4与单例外函数的共同父系综深尾体积比实验：lockstep版。
...
再从同一批parent particles、同一组replica和lineage分出两个条件分支。两条
分支按每个共同请求阈值交替推进；每到一个共同截面立即计算体积比并执行在线
停止判据。若从顺序版checkpoint接管，领先分支等待、落后分支追赶。共同父
...
def main() -> None:
    ...
    parity_status, flip_status, stopping = run_paired_lockstep(
        output_dir, parent, parity_target, flip_target,
        inputs, blocks, parity_eval, flip_eval,
    )
    save_plots(output_dir)
    summary = {
        "status": (
            "completed"
            if parity_status == flip_status == "completed"
            else (
                "stopped_robust_crossing"
                if parity_status == flip_status == "stopped_robust_crossing"
                else "partial"
            )
        ),
