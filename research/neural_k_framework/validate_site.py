...
def main() -> int:
    ...
    if len(pages) != 60:
        errors.append(f"预期 60 个 HTML 页面，实际 {len(pages)} 个。")
    ...
    if experiment_index and experiment_index.cards != 25:
        errors.append(f"实验索引卡片应为 25，实际 {experiment_index.cards}。")
    experiment_index_en = parsed.get((SITE / "en" / "experiments" / "index.html").resolve())
    if experiment_index_en and experiment_index_en.cards != 25:
        errors.append(f"英文实验索引卡片应为 25，实际 {experiment_index_en.cards}。")
    ...
    if main_page:
        ...
        if len(experiment_links) != 25:
            errors.append(f"主文应链接全部 25 个实验页，实际 {len(experiment_links)}。")
