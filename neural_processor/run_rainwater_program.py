# run_rainwater_program.py

import torch
import torch.nn as nn
import json
import random
from tqdm import tqdm
import time
import os

# ==============================================================================
# --- 1. “球状闪电v2.2” 架构定义 (与训练时100%同步) ---
# ==============================================================================
class Config:
    INPUT_BITS = 50; OUTPUT_BITS = 34; HIDDEN_SIZE = 4096; NUM_HIDDEN_LAYERS = 4

class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        layers = [ nn.Linear(config.INPUT_BITS, config.HIDDEN_SIZE), nn.GELU(), nn.LayerNorm(config.HIDDEN_SIZE), nn.Dropout(0.1) ]
        for _ in range(config.NUM_HIDDEN_LAYERS):
            layers.extend([ nn.Linear(config.HIDDEN_SIZE, config.HIDDEN_SIZE), nn.GELU(), nn.LayerNorm(config.HIDDEN_SIZE), nn.Dropout(0.1) ])
        layers.append(nn.Linear(config.HIDDEN_SIZE, config.OUTPUT_BITS))
        self.net = nn.Sequential(*layers)
    def forward(self, x): return self.net(x)

NUM_REGISTERS, BITS_PER_REGISTER, NUM_FLAGS = 4, 8, 2
STATE_BITS, INSTRUCTION_BITS = 34, 16
OPCODES = {
    "NOP": 0, "MOVI": 1, "MOV": 2, "ADD": 3, "SUB": 4, "INC": 5, "DEC": 6, 
    "AND": 7, "OR": 8, "XOR": 9, "NOT": 10, "SHL": 11, "CMP": 12, "HALT": 15
}

class PerfectALU: # 真值引擎
    def parse_instruction(self, i): return ((i>>12)&0xF, (i>>10)&0x3, (i>>8)&0x3, (i>>6)&0x3, i&0xFF)
    def execute(self, i, r, z, g):
        regs, zf, gf = r[:], z, g
        op, p1, p2, p3, imm = self.parse_instruction(i)
        dst, src1, src2 = p1, p2, p3
        if op in [OPCODES["NOP"], OPCODES["HALT"]]: pass
        elif op == OPCODES["MOVI"]: regs[dst] = imm
        elif op == OPCODES["MOV"]: regs[dst] = regs[src1]
        elif op == OPCODES["ADD"]: regs[dst] = (regs[src1] + regs[src2]) & 0xFF
        elif op == OPCODES["SUB"]: regs[dst] = (regs[src1] - regs[src2]) & 0xFF
        elif op == OPCODES["INC"]: regs[dst] = (regs[dst] + 1) & 0xFF
        elif op == OPCODES["DEC"]: regs[dst] = (regs[dst] - 1) & 0xFF
        elif op == OPCODES["AND"]: regs[dst] = regs[src1] & regs[src2]
        elif op == OPCODES["OR"]: regs[dst] = regs[src1] | regs[src2]
        elif op == OPCODES["XOR"]: regs[dst] = regs[src1] ^ regs[src2]
        elif op == OPCODES["NOT"]: regs[dst] = ~regs[src1] & 0xFF
        elif op == OPCODES["SHL"]: regs[dst] = (regs[src1] << (imm & 0x7)) & 0xFF
        elif op == OPCODES["CMP"]:
            zf = 1 if regs[src1] == regs[src2] else 0
            gf = 1 if regs[src1] > regs[src2] else 0
        return regs, zf, gf


# ==============================================================================
# --- 2. “混合智能”CPU模拟器 ---
# ==============================================================================
class HybridNeuralCPU:
    def __init__(self, model_path, config):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"正在加载神经核心 (Neural Core) 并移至 {self.device}...")
        self.neural_alu = MLP(config).to(self.device)
        if os.path.exists(model_path):
            self.neural_alu.load_state_dict(torch.load(model_path, map_location=self.device))
        else:
            print(f"Warning: Model file {model_path} not found. Please train the model first.")
        self.neural_alu.eval()
        
        self.perfect_alu = PerfectALU() # 真值引擎，用于单步验证
        
        # Trapping Rain Water (Two Pointers Algorithm) Assembly
        assembly_program = [
            # --- 初始化 ---
            {"op": "MOVI", "dst": 0, "imm": 0},      # R0(left_ptr) = 0
            {"op": "MOVI", "dst": 1, "imm": 9},      # R1(right_ptr) = 9
            {"op": "MOVI", "dst": 2, "imm": 0},      # R2(left_max) = 0
            {"op": "MOVI", "dst": 3, "imm": 0},      # R3(right_max) = 0
            
            # --- 主循环检查点 ---
            "LOOP_START:",
            {"op": "CMP", "src1": 1, "src2": 0},      # if left_ptr < right_ptr
            {"op": "JG", "target": "LOOP_BODY"},     #   then continue loop
            {"op": "HALT"},                          # else, end program

            # --- 主循环体 ---
            "LOOP_BODY:",
            {"op": "LOAD", "dst": 0, "addr_reg": 0},  # temp_R0 = MEM[left_ptr]
            {"op": "LOAD", "dst": 1, "addr_reg": 1},  # temp_R1 = MEM[right_ptr]
            {"op": "CMP", "src1": 1, "src2": 0},      # if height[left] < height[right]
            {"op": "JG", "target": "LEFT_IS_SHORT"}, #   goto LEFT_IS_SHORT

            # --- 右边是短板的逻辑块 ---
            "RIGHT_IS_SHORT:",
            {"op": "CMP", "src1": 3, "src2": 1},      # if right_max > height[right]
            {"op": "JG", "target": "R_CALC_WATER"},  #   then calculate water
            {"op": "MOV", "dst": 3, "src1": 1},       # else, right_max = height[right]
            "R_CALC_WATER:",
            {"op": "SUB", "dst": 2, "src1": 3, "src2": 1}, # R2(temp_water) = right_max - height[right]
            {"op": "STORE", "src_reg": 2, "addr_reg": 1},# MEM[right_ptr+10] = temp_water (需要地址偏移)
            {"op": "DEC", "dst": 1},                 # right_ptr--
            {"op": "JMP", "target": "LOOP_START"},

            # --- 左边是短板的逻辑块 ---
            "LEFT_IS_SHORT:",
            {"op": "CMP", "src1": 2, "src2": 0},      # if left_max > height[left]
            {"op": "JG", "target": "L_CALC_WATER"},  #   then calculate water
            {"op": "MOV", "dst": 2, "src1": 0},       # else, left_max = height[left]
            "L_CALC_WATER:",
            {"op": "SUB", "dst": 1, "src1": 2, "src2": 0}, # R1(temp_water) = left_max - height[left]
            {"op": "STORE", "src_reg": 1, "addr_reg": 0},# MEM[left_ptr+10] = temp_water
            {"op": "INC", "dst": 0},                 # left_ptr++
            {"op": "JMP", "target": "LOOP_START"},
        ]
        
        # --- 简单的汇编器 ---
        labels = {}; program = []
        for line in assembly_program:
            if isinstance(line, str) and line.endswith(":"):
                labels[line[:-1]] = len(program)
            else:
                program.append(line)
        
        for instr in program:
            if "target" in instr:
                instr["target"] = labels[instr["target"]]
        
        self.program = program
        self.machine_code = [self.assemble(instr) for instr in self.program]
        
        self.total_steps_executed = 0
        self.correct_steps_executed = 0

    def assemble(self,i): op_name=i.get("op","NOP");op=OPCODES.get(op_name,0);return(op<<12)|(i.get("dst",0)<<10)|(i.get("src1",0)<<8)|(i.get("src2",0)<<6)|i.get("imm",0)
    def state_to_vector(self,r,z,g): v=[z,g]; [v.extend(list(map(int,f'{val:08b}'))) for val in r]; return torch.tensor(v,dtype=torch.float32).to(self.device)
    def vector_to_state(self,v): z,g=int(v[0].item()),int(v[1].item()); r=[]; [r.append(int("".join(map(str,v[2+i*8:10+i*8].int().tolist())),2)) for i in range(4)]; return r,z,g
    def parse_instruction(self, instr_int):
        opcode = (instr_int >> 12) & 0xF
        p1     = (instr_int >> 10) & 0x3 # Dst or Src1
        p2     = (instr_int >> 8)  & 0x3 # Src1 or Src2
        p3     = (instr_int >> 6)  & 0x3 # Src2
        imm    = instr_int & 0xFF
        return opcode, p1, p2, p3, imm
    def run_program(self, initial_heights):
        # 初始化状态
        regs, zf, gf, pc = [123]*NUM_REGISTERS, 0, 0, 0
        mem = [0]*22
        mem[:10] = initial_heights[:]

        for _ in range(200): # 设置最大执行步数
            if pc >= len(self.program): break
            instr = self.program[pc]
            op_name = instr["op"]

            # --- Python 控制器处理内存和跳转 ---
            if op_name == "LOAD":
                addr = regs[instr["addr_reg"]]
                if 0 <= addr < len(mem): regs[instr["dst"]] = mem[addr]
                pc += 1; continue
            if op_name == "STORE":
                addr = regs[instr["addr_reg"]]
                if 0 <= addr < len(mem): mem[addr] = regs[instr["src_reg"]]
                pc += 1; continue
            if op_name in ["JMP", "JZ", "JG", "HALT"]:
                if op_name == "HALT": 
                    # print('HALT')
                    break
                if (op_name=="JMP") or (op_name=="JZ" and zf==1) or (op_name=="JG" and gf==1): pc = instr["target"]
                else: pc += 1
                continue

            # --- 神经网络处理计算 ---
            self.total_steps_executed += 1
            machine_code = self.machine_code[pc]
            
            with torch.no_grad():
                # Prediction
                true_regs, true_zf, true_gf = self.perfect_alu.execute(machine_code, regs, zf, gf)
                
                instr_tensor = torch.tensor([int(b) for b in format(machine_code,f'0{INSTRUCTION_BITS}b')],dtype=torch.float32).to(self.device)
                state_tensor = self.state_to_vector(regs, zf, gf)
                input_vector = torch.cat((instr_tensor, state_tensor)).unsqueeze(0)
                output_vector = self.neural_alu(input_vector).squeeze(0)
                preds_vector = (torch.sigmoid(output_vector) > 0.5).int().to(self.device)
                
                true_vector = self.state_to_vector(true_regs, true_zf, true_gf).int().to(self.device)
                
                if not torch.all(preds_vector.int() == true_vector):
                    return [-1] * 10 # 单步出错，程序失败

                self.correct_steps_executed += 1
                regs, zf, gf = self.vector_to_state(preds_vector)
            pc += 1
        
        return mem[10:20] # 返回存储在内存中的雨水结果

# ==============================================================================
# --- 3. 最终验证流程 ---
# ==============================================================================

def solve_trapping_rain_water_python(height):
    n = len(height); water = [0]*n
    if n==0: return water
    left_max=[0]*n
    left_max[0]=height[0]
    for i in range(1,n):
        left_max[i]=max(left_max[i-1],height[i])
    right_max=[0]*n
    right_max[n-1]=height[n-1]
    for i in range(n-2,-1,-1):
        right_max[i]=max(right_max[i+1],height[i])
    for i in range(n):
        if min(left_max[i],right_max[i])>height[i]:
            water[i]=min(left_max[i],right_max[i])-height[i]
    return water

def final_validation(num_tests=1):
    print("\n--- 开始端到端'接雨水'程序执行与单步准确率验证 ---")
    config = Config()
    # Note: Using likely compatible model name
    emulator = HybridNeuralCPU("best_model_mlp_cpu2.pth", config)
    
    program_correct_count = 0
    with tqdm(total=num_tests, desc="测试接雨水程序") as pbar:
        for i in range(num_tests):
            heights = [random.randint(0, 7) for _ in range(10)]
            ground_truth = solve_trapping_rain_water_python(heights)
            
            neural_result = emulator.run_program(heights)
            
            if neural_result == ground_truth: program_correct_count += 1
            pbar.update(1)
            pbar.set_postfix_str(f"程序准确率: {100*program_correct_count/(i+1):.4f}%")

    total_steps = emulator.total_steps_executed if emulator.total_steps_executed > 0 else 1
    program_accuracy = 100 * program_correct_count / num_tests
    step_accuracy = 100 * emulator.correct_steps_executed / total_steps
    
    print("\n=================================================")
    print("                最终验证结果报告")
    print("=================================================")
    print(f"  - 测试程序: 10根3-bit柱子接雨水算法")
    print(f"  - 总测试程序数: {num_tests:,}")
    print(f"  - 总执行计算指令数: {emulator.total_steps_executed:,}")
    print("---")
    print(f"  - 单步执行准确率 (Per-Step Accuracy): {step_accuracy:.12f}%")
    print(f"  - 完整程序执行准确率 (End-to-End Accuracy): {program_accuracy:.6f}%")
    print("=================================================")

if __name__ == '__main__':
    final_validation()
