"""
让训练好的 Neural CPU v3 执行一个真正的圆周率汇编程序。

程序特点：
- 神经网络逐条执行 16-bit 机器指令；PC、取指和 64-KiB RAM 由精确控制器维护。
- 汇编程序使用 Rabinowitz-Wagon 十进制 spigot 算法，运算本身只依赖本项目 ISA。
- 每一次神经 CPU 调用都同时计算精确真值；第一次出现任何状态位或控制位差异时立即停止。
- 每产生一个新十进制数字，就打印当前 pi、已输出位数和与高精度参考值一致的有效位数。

说明：当前汇编实现的 spigot 数组元素为 16 bit，可能溢出的临时累加器 X 为
24 bit。默认计算 1000 个有效数字；CPU 推理本身不设步数上限，会一直运行到程序
完成、用户中断，或首次神经执行错误。所有运行参数都集中在文件顶部的
``Config`` 中。
"""
...
class Config:
    ...
    RESULT_DIR = BASE_DIR / "results" / "pi"

    TARGET_DECIMAL_DIGITS = 1000
    # 1000 位约需数十亿次操作；默认用流式生成真值轨迹 + GPU 大 batch 审计。
    # serial 仍可显式启用，但在单 lane 上完成 1000 位会非常慢。
    EXECUTION_MODE = "batch"
    EXACT_ONLY = False
    TEACHER_FORCE_ON_ERROR = True
    # 1000 位需要数十亿次机器操作，默认不能沿用早期冒烟测试的 100 万步上限。
    _MAX_OPS = 0
    MAX_NEURAL_OPERATIONS: int | None = _MAX_OPS if _MAX_OPS > 0 else None

    # teacher-forced 审计只打印前若干个错误，之后按固定间隔打印；完整现场始终写入 JSONL。
    PRINT_FIRST_ERRORS = 20
    PRINT_EVERY_ERRORS = 100

    # 每产生一个数字就打印。输出很长时可改成 5、10 等。
    PRINT_EVERY_DIGITS = 1
    PROGRESS_EVERY_OPERATIONS = 1_000_000
    SAVE_PROGRESS_EVERY_DIGITS = 10

    # 与训练保持一致：FP32、关闭 TF32，不使用 AMP。
    ALLOW_TF32 = False
    USE_CUDA_GRAPH = True

    # batch 模式先由原生真值解释器流式生成轨迹，再用大 batch 审计神经核。
    # TRACE_BATCH 控制每块轨迹长度；INFERENCE_BATCH 控制单次 GPU 前向规模。
    TRACE_BATCH = 262_144
...
def main() -> None:
    if not 1 <= Config.TARGET_DECIMAL_DIGITS <= 5000:
        raise ValueError("Config.TARGET_DECIMAL_DIGITS 必须在 1..5000 之间。")
    if Config.EXECUTION_MODE not in {"serial", "batch"}:
        raise ValueError("Config.EXECUTION_MODE 只能是 serial 或 batch。")
    if Config.EXECUTION_MODE == "batch" and Config.EXACT_ONLY:
        raise ValueError("batch 模式用于审计神经权重，不能同时设置 EXACT_ONLY=1。")
    ...
    if not Config.EXACT_ONLY and device.type != "cuda":
        print("警告：逐指令 MLP 推理在 CPU 上会非常慢。")

    builder = PiAssemblyBuilder(Config.TARGET_DECIMAL_DIGITS)
    program, listing = builder.build()
    Config.RESULT_DIR.mkdir(parents=True, exist_ok=True)
    (Config.RESULT_DIR / "pi_program_listing.json").write_text(
        json.dumps(listing, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (Config.RESULT_DIR / "pi_program.bin").write_bytes(
        b"".join(word.to_bytes(2, "big") for word in program)
    )

    reference = reference_pi_digits(Config.TARGET_DECIMAL_DIGITS + 5)
    runner = CoreRunner(device)
    if Config.EXECUTION_MODE == "batch":
        machine = BatchedNeuralPiAuditor(program, listing, runner, reference)
    else:
        machine = NeuralPiMachine(program, listing, runner, reference)
    machine.run()
