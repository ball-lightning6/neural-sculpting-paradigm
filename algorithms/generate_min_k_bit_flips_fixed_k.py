...
TRAIN_FILE = f'min_k_flips_binary_n{NUMS_LENGTH_N}_k{K_MAX_N}_fixed_train.jsonl'
...
def generate_datasets(num_samples, n, k_max, k_bits, output_bits):
    ...
    train_data, eval_data = records[:split_idx], records[split_idx:]

    def write_to_file(data, path):
        print(f"\n正在写入 {len(data)} 条数据到 '{path}'...")
        with open(path, 'w') as f:
            for record in data: f.write(json.dumps(record) + '\n')

    write_to_file(train_data, TRAIN_FILE)
    write_to_file(eval_data, EVAL_FILE)
    print("\n所有数据集生成完成！")
