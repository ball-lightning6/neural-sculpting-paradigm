...
### 1. 配置参数 ###
DATASET_DIR = "arc_jigsaw_puzzle_masterpiece_dataset"
GRID_DIM = 28
CELL_SIZE = 8
...
def rotate_and_scale(piece_mask, rotation):
...
def generate_logic_grids():
    for _ in range(100):
        ...
        for _ in range(num_source_pieces):
            piece = generate_piece(1,6,piece_size_exclude_set)
        ...
        for info in source_pieces_info:
            ...
            scaled_piece = rotate_and_scale(info['shape'], rotation)
        ...
        for _ in range(num_noise_pieces):
            noise_piece = generate_piece(1,2)
        ...
        for info in transformed_pieces_info:
            ...
            scaled_original = rotate_and_scale(original_piece, 0)
            color = info['color']
