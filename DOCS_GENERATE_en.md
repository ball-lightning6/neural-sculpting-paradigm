...
- **Purpose:** Generates a **decoupled version** of the Tower of Hanoi dataset, decomposing the complex recursive strategy learning into two simpler subtasks: predicting each disk's intermediate target pillar (scaffold) and predicting the next action. **This dataset has been verified to successfully train MLP models.**

- **Logic:** 
  - The core innovation is introducing "intermediate goals" (scaffold) as intermediate supervision signals. For any state, the algorithm recursively calculates each disk's temporary target pillar: the largest disk's target is fixed to the destination pillar, while other disks' targets depend on whether their parent disk is already in place.
  - If the parent disk is at its target position, the current disk shares the same target; otherwise, the current disk must move to the "buffer pillar" of the parent disk to make way.
  - Action labels are generated by comparing the current state with intermediate goals: find the first disk not at its target position and output the action it should execute.

- **I/O Format:**

  - Input: State vector representing each disk's current pillar position (list format, e.g., `[-1, -1, 0, 1]`, where -1/0/1 represent three pillars).
    
  - Output: Dictionary containing two fields:
    - `scaffold`: One-hot encoding of each disk's intermediate target pillar (3-dim × N disks, flattened to 3N-dim vector).
    - `action`: One-hot encoding of the next action (6-dim vector representing 6 possible pillar-to-pillar moves).

- **Training Feasibility:** This decoupled representation has been experimentally verified to enable successful training. MLP models can first learn to predict intermediate goals (decomposing complex long-term planning into local subgoals), then decide actions based on the discrepancy between goals and current state, significantly reducing learning difficulty.

- **Main Parameters:** num_disks, num_samples, target_peg (default 1).
